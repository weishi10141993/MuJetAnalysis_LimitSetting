...
def limit_CSxBR2xAlpha_fb_vs_mGammaD():
    ...
    for m in np.arange(m_SR1_min, m_SR1_max, 0.005):
        ...
        CSxBR2xAlpha_fb_fit_SR1.append(( m, ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/eFullMc_over_aGen )) # use interpolation
    for m in np.arange(m_SR2_min, m_SR2_max, 0.005):
        #CSxBR2xAlpha_fb_fit_SR2.append(( m, fCmsLimitVsM(m)/lumi_fbinv/SF/eFullMc_over_aGen ))
        CSxBR2xAlpha_fb_fit_SR2.append(( m, ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/eFullMc_over_aGen ))
    ###########################################################
    ## SR3: don't plot this region if do combine for 2016+2018
    ###########################################################
    if year != 2020:
    ...
    l_CSxBR2xAlpha_fb_fit = ROOT.TLegend(0.25, 0.8, 0.75, 0.9)
    ...
    gr_CSxBR2xAlpha_fb_fit_SR1.Draw("C") # observed limit
    ## SR2
    ...
    l_CSxBR2xAlpha_fb_toy_Brazil_Bands = ROOT.TLegend(0.25, 0.7, 0.6, 0.9)
    ...
    l_CSxBR2xAlpha_fb_toy_Brazil_Bands.SetHeader("%d%% CL upper limits"%CL, "C")
    l_CSxBR2xAlpha_fb_toy_Brazil_Bands.AddEntry(gr_CSxBR2xAlpha_fb_fit_SR1, "Observed", "L")
...
def limit_CSxBR2_fb_vs_ma():
    ...
    for ma_i in array_ma: #defined in UserInput.py
        array_ma_mh_90.append(( ma_i, ExpectedLimitVsM_HybridNew(ma_i, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsNmssmExtrapolate(ma_i, 90.,  CmsNmssmAcceptance) ))
        array_ma_mh_125.append(( ma_i, ExpectedLimitVsM_HybridNew(ma_i, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsNmssmExtrapolate(ma_i, 125., CmsNmssmAcceptance) ))
        array_ma_mh_150.append(( ma_i, ExpectedLimitVsM_HybridNew(ma_i, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsNmssmExtrapolate(ma_i, 150., CmsNmssmAcceptance) ))
...
def limit_CSxBR2_fb_vs_mh():
    ...
    for mh_i in array_mh:
        array_mh_CSxBR_NMSSM_ma_0p5.append(( mh_i, ExpectedLimitVsM_HybridNew(0.5, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsNmssmExtrapolate(0.5, mh_i, CmsNmssmAcceptance) )) # Model Independent limits transformed to Xsec
        array_mh_CSxBR_NMSSM_ma_3.append(( mh_i, ExpectedLimitVsM_HybridNew(3, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsNmssmExtrapolate(3, mh_i, CmsNmssmAcceptance) ))
    ...
    l_CSxBR_NMSSM_vs_mh.SetHeader("NMSSM %d%% CL upper limits:"%CL)
...
def limit_ALP_Higgs_vs_ma():
    ...
    for m in np.arange(0.5, m_SR1_max, 0.005):
        CahEff_over_LambdaSquare_TeV_LeptBr_1_fit_SR1.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
        CahEff_over_LambdaSquare_TeV_LeptBr_0p1_fit_SR1.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_0p1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
    for m in np.arange(m_SR2_min, m_SR2_max, 0.005):
        CahEff_over_LambdaSquare_TeV_LeptBr_1_fit_SR2.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
        CahEff_over_LambdaSquare_TeV_LeptBr_0p1_fit_SR2.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_0p1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
    for m in np.arange(m_SR3_min, 30, 0.005):
        CahEff_over_LambdaSquare_TeV_LeptBr_1_fit_SR3.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
        CahEff_over_LambdaSquare_TeV_LeptBr_0p1_fit_SR3.append(( m, math.sqrt( (HiggsSMWidthMeV/1000.)/(1./(ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000)/Br_ALP_Lepton_0p1**2) - 1)*32*3.1415/(HiggsVEV**2)/(HiggsMass**3)/( (1 - 2*(m/HiggsMass)**2)**2 )/math.sqrt(1 - 4*(m/HiggsMass)**2) )*10**6 ))
    ...
    l_CahEff_over_LambdaSquare_TeV_fit.SetHeader("ALP %d%% CL upper limits:"%CL)
...
def limit_ALP_Lepton_vs_ma():
    ...
    for m in np.arange(0.5, m_SR1_max, 0.005):
        clleff_over_Lambda_TeV_Cah_1_fit_SR1.append(( m, math.sqrt( ( 8*3.1415/MuMassGeV**2/m/math.sqrt( 1-4*(MuMassGeV/m)**2 ) )*fCmsAlpExtrapolate(m, ALPTotWidthGeV)*math.sqrt( 1+ HiggsSMWidthMeV*32*3.1415/1000./( (CahEff_over_LambdaSquare_TeV_1/10**6)**2*HiggsVEV**2*HiggsMass**3*(1 - 2*(m/HiggsMass)**2)**2*math.sqrt(1 - 4*(m/HiggsMass)**2) ) )*ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000) )*10**3 ))
        clleff_over_Lambda_TeV_Cah_0p1_fit_SR1.append(( m, math.sqrt( ( 8*3.1415/MuMassGeV**2/m/math.sqrt( 1-4*(MuMassGeV/m)**2 ) )*fCmsAlpExtrapolate(m, ALPTotWidthGeV)*math.sqrt( 1+ HiggsSMWidthMeV*32*3.1415/1000./( (CahEff_over_LambdaSquare_TeV_0p1/10**6)**2*HiggsVEV**2*HiggsMass**3*(1 - 2*(m/HiggsMass)**2)**2*math.sqrt(1 - 4*(m/HiggsMass)**2) ) )*ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000) )*10**3 ))
        clleff_over_Lambda_TeV_Cah_0p01_fit_SR1.append(( m, math.sqrt( ( 8*3.1415/MuMassGeV**2/m/math.sqrt( 1-4*(MuMassGeV/m)**2 ) )*fCmsAlpExtrapolate(m, ALPTotWidthGeV)*math.sqrt( 1+ HiggsSMWidthMeV*32*3.1415/1000./( (CahEff_over_LambdaSquare_TeV_0p01/10**6)**2*HiggsVEV**2*HiggsMass**3*(1 - 2*(m/HiggsMass)**2)**2*math.sqrt(1 - 4*(m/HiggsMass)**2) ) )*ExpectedLimitVsM_HybridNew(m, Observed_Limits_HybridNew)/lumi_fbinv/SF/fCmsAlpExtrapolate(m, CmsAlpAcceptance)/(ggHXsecpb*1000) )*10**3 ))
    for m in np.arange(m_SR2_min, m_SR2_max, 0.005):
    ...
    l_clleff_over_Lambda_TeV_Cah_fit.SetHeader("ALP %d%% CL upper limits:"%CL)
